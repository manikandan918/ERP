...
useEffect(()=>{
  ...
  const handleFormSubmit = (event) => {
    ...
    if (editId) {
      ...
      axios.put(`http://127.0.0.1:8000/api/updatepayroll/${editId}`, data)
        .then(response => {
      ...
      axios.post('http://127.0.0.1:8000/api/insertpayroll', data)
        .then(response => {
          console.log('Response:', response);
  ...
  const handleDelete = (id) => {
    axios.delete('http://127.0.0.1:8000/api/deleteHRM2/' + id)
        .then(() => {
...
const handleEdit = (id) => {
  ...
  if (selectedPost) {
    ...
    setpayrolltype(selectedPost.payrolltype);
    setsalary(selectedPost.salary);
    setnetsalary(selectedPost.netsalary);
    setShow(true);
  }
};
            ...
            <a className="nav-link active nav-itemcollapsed" data-bs-toggle="collapse" href="#hrm-list" >
              <i className="me-2"><FontAwesomeIcon icon={faBox} /></i>HRM<i className="bi bi-chevron-down mt-2" style={{ marginTop: "2px" }} ></i>        
                   ...
                   <ul id="hrm-list" className="nav-content collapse" data-bs-parent="#sidebar-nav">
                      ...
                      <Link to="/Payroll" className="nav-item nav-link active" >
          ...
          <div className="container-fluid pt-4 px-4">
            <div className="bg-light text-center rounded p-4">
              <div className="d-flex align-items-center justify-content-between mb-4">
                <h6 className="mb-0">Payroll list</h6>
              ...
              <Modal show={show} onHide={handleClose}>
                ...
                <Modal.Body>
                  <Form onSubmit={handleFormSubmit}>
                    ...
                    <div className="mb-3">
                      <label htmlFor="Date" className="form-label">Payroll type</label>
                      <input type="email" className="form-control" id="payrolltype" name="payrolltype" value={payrolltype} onChange={handleChange} required />
                    </div>
                    
                    <div className="mb-3">
                      <label htmlFor="email" className="form-label">Salary:</label>
                      <input type="text" className="form-control" id="salary" name="salary" value={salary} onChange={handleChange} required />
                      </div>
                      <div className="mb-3">
                      <label htmlFor="email" className="form-label">Net Salary:</label>
                      <input type="text" className="form-control" id="netsalary" name="netsalary" value={netsalary} onChange={handleChange} required />
                      </div>
                  ...
                  <tbody>
                    {posts.map((post, index) => (
                      <tr key={index}>
                        ...
                        <td>{post.name}</td>
                        <td>{post.payrolltype}</td>
                        <td>{post.salary}</td>
                        <td>{post.netsalary}</td>
                        <td><button onClick={() => handleEdit(post.id)} className="btn btn-sm btn-warning ms-2 "><FontAwesomeIcon icon={faPencilAlt} /></button>
                            <button onClick={() => handleDelete(post.id)} className="btn btn-sm btn-danger ms-4 me-2"><FontAwesomeIcon icon={faTrashAlt} /></button>
                        </td>
                      </tr>
                    ))}
                  </tbody>
